...
###   @LastEditors: AceSix
###   @LastEditTime: 2022-12-15 18:01:54
...
def gen_interp_video(G, triplanes, mp4: str, seeds, shuffle_seed=None, w_frames=60*4, kind='cubic', grid_dims=(1,1), num_keyframes=None, wraps=2, psi=1, truncation_cutoff=14, cfg='FFHQ', image_mode='image', gen_shapes=False, device=torch.device('cuda'), **video_kwargs):
    grid_w = grid_dims[0]
    grid_h = grid_dims[1]
    ...
    _ = G.synthesis(triplanes, c[:1], ws=ws[:1]) # warm up
        ...
        for yi in range(grid_h):
            for xi in range(grid_w):
                ...
                cam2world_pose = LookAtPoseSampler.sample(3.14/2 + yaw_range * np.sin(2 * 3.14 * frame_idx / (num_keyframes * w_frames)),
                                                        3.14/2 -3.14/2 - 0.05 + pitch_range * np.cos(2 * 3.14 * frame_idx / (num_keyframes * w_frames)),
                ...
                if entangle == 'conditioning':
                    ...
                    img = G.synthesis(triplanes, c[:1], ws=ws[:1])[image_mode][0]
                    print(img.shape)
                elif entangle == 'both':
                    w_c = G.mapping(z=zs[0:1], c=c[0:1], truncation_psi=psi, truncation_cutoff=truncation_cutoff)
...
@click.option('--image_mode', help='Image mode', type=click.Choice(['image', 'image_depth', 'image_raw']), required=False, metavar='STR', default='image_raw', show_default=True)
...
def generate_images(
    ...
    if "shape" in network_pkl:
        triplane = torch.load("./logs/car_steps/final_triplane.pth")
    elif "cat" in network_pkl:
        triplane = torch.load("./logs/cat_steps/final_triplane.pth")
