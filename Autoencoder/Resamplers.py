...
class InterpolativeUpsamplerReference(nn.Module):
    ...
    def forward(self, x):
        ...
        y = nn.functional.conv_transpose2d(y.view(y.shape[0] * y.shape[1], 1, y.shape[2], y.shape[3]), Kernel, stride=2, padding=3 * self.FilterRadius)
...
class InterpolativeDownsamplerReference(nn.Module):
    ...
    def forward(self, x):
        ...
        y = nn.functional.pad(x, (self.FilterRadius, self.FilterRadius, self.FilterRadius, self.FilterRadius), mode='reflect')
        y = nn.functional.conv2d(y.view(y.shape[0] * y.shape[1], 1, y.shape[2], y.shape[3]), Kernel, stride=2)
...
class InplaceUpsamplerReference(nn.Module):
    ...
    def forward(self, x):
        ...
        return nn.functional.conv2d(y.view(y.shape[0] * y.shape[1], 1, y.shape[2], y.shape[3]), Kernel, stride=1).view(*x.shape)
...
class InplaceDownsamplerReference(nn.Module):
    ...
    def forward(self, x):
        ...
        y = nn.functional.pad(x, (self.FilterRadius, self.FilterRadius, self.FilterRadius, self.FilterRadius), mode='reflect')
        y = nn.functional.conv2d(y.view(y.shape[0] * y.shape[1], 1, y.shape[2], y.shape[3]), Kernel, stride=1).view(*x.shape)
